...
class BipedSim:
    ...
    """
    """
    def dynamixel_ID(self): #identification of dynamixel motors
        self.all_motors = Ax12(254)
        self.ankleL = Ax12(1)
        self.kneeL = Ax12(2)
        self.hipL = Ax12(3)
    ...
    def getNormalForce_modi(self):
        ...
        f2a = p.getContactPoints(robot.robot_id,robot.planeID,1,-1)

        f1b = p.getContactPoints(robot.robot_id,robot.planeID,5,-1)
        f2b = p.getContactPoints(robot.robot_id,robot.planeID,2,-1)
    ...
    def runSim(self):
        ...
        #dth = np.delete(robot.qdot,[0,4])
            ...
            #self.thpos_n = u2d2commcode.get_angle(hipL)

            self.thpos_n = np.array([u2d2commcode.get_angle(hipL), u2d2commcode.get_angle(kneeL), u2d2commcode.get_angle(ankleL),u2d2commcode.get_angle(hipR), u2d2commcode.get_angle(kneeR), u2d2commcode.get_angle(ankleR)])
            
            self.thpos_n = np.reshape(self.thpos_n,(6,))

          #  dth = (self.thpos_n-thprev)/self.t_step1
        ...
        #dthact=dth.T
         ...
         #   x1=vbase_l+self.skewM(robot.link_omega[9])@(-robot.urdf_pos[9]+robot.com_pos[9])
          ...
          #  p.setJointMotorControlArray(robot.robot_id,robot.rev,p.POSITION_CONTROL, forces=np.zeros(len(robot.rev)))
            ...
            tau = np.reshape(tau,(13,))

            #self.jtorques = tau[7:15]

            jointstates = p.getJointStates(robot.robot_id,robot.rev)

            self.thpos_deg =  [(state[0]*6*180/np.pi) for state in jointstates]
...
#            t4 = self.thpos_deg[3], t5 = self.thpos_deg[4], t6 = self.thpos_deg[5]
           ...
           # self.update_servo_pos(t1,t2,t3,t4,t5,t6)
            ...
            if t<self.t_impact+0.3:
               ...
               p.setJointMotorControl2(robot.robot_id, jointIndex = robot.rev[2] ,
                                        controlMode = p.TORQUE_CONTROL, 
                                        force =  self.jtorques[2] - ankle_torque_2-1.2)

               p.setJointMotorControl2(robot.robot_id, jointIndex = robot.rev[5] ,
                                        controlMode = p.TORQUE_CONTROL, 
                                        force =  self.jtorques[5] - ankle_torque_1-1.2) 
            ...
            if area<1e-2:
                ...
                if t<T1:
                    ...
                    p.setJointMotorControl2(robot.robot_id,robot.rev[3],p.POSITION_CONTROL,targetPosition = phi)
            """
        ...
        #dth = np.delete(robot.qdot,[0,4])
           ...
           # dth = (self.thpos_n-thprev)/self.t_step2
