...
# This is for performing static stability experiments
class Biped:
    ...
    def startSim(self,table=True):
        ...
        self.urdf_path = "/home/udayan/Desktop/Alinjar/Biped Pybullet/bipedal_12_01-20220809T113418Z-001/bipedal_12_01/bipedal_4.urdf" #bipedal_4 for static stability
        ...
        self.z_lev = 0.08
        self.robot_id = p.loadURDF(self.urdf_path,useFixedBase= False, basePosition=[0,-0.045,self.z_lev])
        
        #self.urdf_path_cylinder = "/home/udayan/Desktop/Alinjar/Biped Pybullet/bipedal_12_01-20220809T113418Z-001/bipedal_12_01/cylinder.urdf" #bipedal_4 for static stability
        self.urdf_path_sphere = "/home/udayan/Desktop/Alinjar/Biped Pybullet/bipedal_12_01-20220809T113418Z-001/bipedal_12_01/sphere.urdf" #bipedal_4 for static stability

        #self.cylinder_id = p.loadURDF(self.urdf_path_cylinder,useFixedBase= True, basePosition=[-0.1,-0.25,0.025])
        self.sphere_id = p.loadURDF(self.urdf_path_sphere,useFixedBase= True, basePosition=[-0.1,-0.25,0.025])
      #  self.robot_id_fixed = p.loadURDF(self.urdf_path,useFixedBase= True, basePosition=[0,0,0])
        self.mu =0.4 #0.2 for no sliding result
        p.changeDynamics(self.planeID ,  -1, lateralFriction=self.mu) #keep it 0.6 for no tipping result
        #p.changeDynamics(self.cylinder_id ,  -1, lateralFriction=self.mu)
    ...
    def drawPbtwopoint(self, point1, point2):
        if self.lineptr != None:
                p.removeUserDebugItem(self.lineptr)
        
        print("point1", point1)        # pass
        self.lineptr = p.addUserDebugLine([point1[0], point1[1], self.z_lev+0.01],[point2[0], point2[1], self.z_lev+0.01],lineColorRGB=[0,0,1],lineWidth=50)       
