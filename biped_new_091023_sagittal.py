...
class Biped:
    ...
    def startSim(self,table=True):
        ...
        #self.urdf_path = "Biped_12DOF_200923.urdf"
       ...
       # for normal view
        ...
        p.resetDebugVisualizerCamera(cameraDistance = 0.9, cameraYaw= 90, cameraPitch = 0, cameraTargetPosition = [1,1,0.4]) #sagittal plane
       # p.resetDebugVisualizerCamera(cameraDistance = 1, cameraYaw= 0, cameraPitch = 0, cameraTargetPosition = [1,1,0.4]) #frontal plane
       # for experimentation  
       # p.resetDebugVisualizerCamera(cameraDistance = 0.5, cameraYaw= 0, cameraPitch = 0, cameraTargetPosition = [1,1,0.4]) 
    ...
    def getJointState(self):
      ...
      #  print("qdot:",self.qdot)
        ...
        for iii in range(len(self.allj)):  
          self.jaxes.append(p.getJointInfo(self.robot_id,iii)[13])
        
      #   print("self.jaxes[4]", self.jaxes[4]) #hip yaw
      #   print("self.jaxes[7]", self.jaxes[7]) #hip roll
      #   print("self.jaxes[12]", self.jaxes[12]) # hip pitch
      #   print("self.jaxes[16]", self.jaxes[16]) # knee pitch
      #   print("self.jaxes[17]", self.jaxes[17]) # ankle pitch
      #   print("self.jaxes[24]", self.jaxes[24]) # ankle roll
      #   print("self.jaxes[27]", self.jaxes[27]) #hip yaw
      #   print("self.jaxes[30]", self.jaxes[30]) #hip roll
      #   print("self.jaxes[35]", self.jaxes[35]) # hip pitch
      #   print("self.jaxes[39]", self.jaxes[39]) # knee pitch
      #   print("self.jaxes[40]", self.jaxes[40]) # ankle pitch
      #   print("self.jaxes[47]", self.jaxes[47]) # ankle roll
      # #  print("jdetails:",self.jdetails)
        #print("jaxes",self.jaxes)
    ...
    def setLinkM(self):
       ...
       # link_mass = p.getDynamicsInfo(self.robot_id,  -1)[0]
        ...
        #print(p.getDynamicsInfo(self.robot_id ,48))
    ...
    def getCOMPos(self):
       ...
       # linkmass_data  = p.getDynamicsInfo(self.robot_id,  0)
        ...
        rshank_length =np.linalg.norm(self.urdf_pos[17]-self.urdf_pos[16]) 

        rthigh_length =np.linalg.norm(self.urdf_pos[16]-self.urdf_pos[12]) 

        lshank_length =np.linalg.norm(self.urdf_pos[40]-self.urdf_pos[39]) 

        lthigh_length =np.linalg.norm(self.urdf_pos[39]-self.urdf_pos[35]) 

        # print("rshank_length", rshank_length)
        # print("rthigh_length", rthigh_length)
        # #print("lshank_length", lshank_length)
        # print("lshank_length", lshank_length)
        # print("lthigh_length", lthigh_length)

        # self.drawPb1(self.urdf_pos[17]) #right ankle
        # time.sleep(10)
        # self.drawPb1(self.urdf_pos[16]) #right knee
        # time.sleep(10)
        # self.drawPb1(self.urdf_pos[12]) #right hip
        # time.sleep(10)
    ...
    def drawPbPointgreen(self, point):
       # if self.lineptr != None:
             #   p.removeUserDebugItem(self.lineptr)
                # pass
        self.lineptr = p.addUserDebugLine(point,point-[0.001,0.001,0.001],lineColorRGB=[0,1,0],lineWidth=5)    
