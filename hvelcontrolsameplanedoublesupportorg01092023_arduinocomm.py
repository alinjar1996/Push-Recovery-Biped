        ...
        #dthact=dth.T
         ...
         #   x1=vbase_l+self.skewM(robot.link_omega[9])@(-robot.urdf_pos[9]+robot.com_pos[9])
          ...
          #  p.setJointMotorControlArray(robot.robot_id,robot.rev,p.POSITION_CONTROL, forces=np.zeros(len(robot.rev)))
            ...
            self.ArduinoWrite(','.join(map(str, [(state[0]*4*180/np.pi) for state in jointstates])))
            #self.ArduinoWrite(','.join(map(str, [5,5,5,0,0,0,0,0])))
           # print("JOINT ANGLES: ", self.ArduinoRead())
            self.ArduinoRead()
            #print(self.ArduinoRead())
            ...
            if t<self.t_impact+0.3:
               ...
               p.setJointMotorControl2(robot.robot_id, jointIndex = robot.rev[3] ,
                                        controlMode = p.TORQUE_CONTROL, 
                                        force =  self.jtorques[3] - ankle_torque_2+2.75)

               p.setJointMotorControl2(robot.robot_id, jointIndex = robot.rev[7] ,
                                        controlMode = p.TORQUE_CONTROL, 
                                        force =  self.jtorques[7] - ankle_torque_1+2.75) 
           ...
           # print("JOINT ANGLES: ", self.ArduinoRead())
            ...
            if t<self.t_impact+2:
               ...
               p.setJointMotorControl2(robot.robot_id, jointIndex = robot.rev[4] ,
                                        controlMode = p.TORQUE_CONTROL, 
                                        force =  self.jtorques[3] - ankle_torque_2+2.5)

               p.setJointMotorControl2(robot.robot_id, jointIndex = robot.rev[10] ,
                                        controlMode = p.TORQUE_CONTROL, 
                                        force =  self.jtorques[7] - ankle_torque_1+2.5) 
