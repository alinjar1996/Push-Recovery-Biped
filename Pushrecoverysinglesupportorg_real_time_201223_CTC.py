...
import time
import numpy as np
from biped_new_torso_appended_180923 import Biped
...
class BipedSim:
    ...
    def getNormalForce_modi(self):
        ...
        """
        f1a = p.getContactPoints(robot.robot_id,robot.planeID,48,-1)

        f2a = p.getContactPoints(robot.robot_id,robot.planeID,25,-1)

        f1b = p.getContactPoints(robot.robot_id,robot.planeID,49,-1)

        f2b = p.getContactPoints(robot.robot_id,robot.planeID,26,-1)
       ...
       # maxlenf1 = max(len(f1a),len(f1b))
        ...
        ankle_joint_pos = robot.urdf_pos[49]
        ...
        ankle_joint_pos = robot.urdf_pos[26] 
    ...
    def runSim(self):
        ...
        p.changeDynamics(robot.robot_id ,  0, mass=0)
        ...
        #ja = p.calculateInverseKinematics(robot.robot_id,-1, targetPosition = tuple(b_p+np.array([0,0,0.5])))
            ...
            self.thdes[2] = 1.4*0.6
            self.thdes[3] = -1.4*0.6
            #thdes[6] = 0.1
        ...
        p.changeDynamics(robot.robot_id ,  0, mass=0.0001) #Included a small box at the top of torso. Do not need that with the current control law.
        #Therefore given a very small value.
           ...
           # p.setRealTimeSimulation(0)
            ...
            if t > self.t_set+self.t_rest and t < self.t_impact+self.t_set+self.t_rest:                             #This combination works the best comparison to v=[000000]
              ...
              # p.applyExternalForce(robot.robot_id,-1,[-0,-9.5,-0],[0,0,0],flags=p.LINK_FRAME) #keep this value
               ...
               self.F_app = np.array([-12.85*0.8/4,-31*0.8/4,0]) 
       ...
       # for i in range(15):
          ...
          # self.linkmass=p.getDynamicsInfo(robot.robot_id, i)
            ...
            self.v_base= (robot.m_array[0]*robot.link_vel[0]+robot.m_array[1]*robot.link_vel[1]+\
                         ...
                         robot.m_array[4]*robot.link_vel[4]+robot.m_array[27]*robot.link_vel[27]+\
                         robot.m_array[50]*self.v_torso/(robot.m_array[0]+robot.m_array[1]+robot.m_array[2]+\
                         robot.m_array[3]+robot.m_array[4]+robot.m_array[27]+robot.m_array[50]))           
        ...
        #vbase_r= robot.base_vel+self.skewM(robot.base_angvel)@(robot.urdf_pos[0]--robot.com_base_pos)
         ...
         # modified for new urdf
            ...
            vbase_r= robot.link_vel[4]+self.skewM(robot.link_omega[4])@(robot.urdf_pos[5]-robot.com_pos[4])
            vbase_l= robot.link_vel[27]+self.skewM(robot.link_omega[27])@(robot.urdf_pos[28]-robot.com_pos[27])
            ...
            Mr = np.sum(robot.setLinkM()[5:27])
            Ml = np.sum(robot.setLinkM()[28:50])

            #M_base= robot.setLinkM()[0]+robot.setLinkM()[17]+robot.setLinkM()[1]+robot.setLinkM()[9]
            M_base= robot.m_array[0]+robot.m_array[1]+robot.m_array[2]+robot.m_array[3]+robot.m_array[4]+\
                    robot.m_array[27]+robot.m_array[50]
            ...
            mv=np.zeros((51,3)) #len(robot.m_array) = 18 for new urdf
       ...
       #m_vcm = np.sum(np.tile(robot.m_array,(3,1))*link_vell.T,axis=1)
        ...
        #self.h = np.array([m_vcm - self.M*self.v_base]).T@np.linalg.pinv([dth]).T
           ...
           # p.setJointMotorControlArray(robot.robot_id,robot.rev,p.POSITION_CONTROL,targetPositions = self.thdes,
            ...
            mgc1 = (self.c_cm[1]-robot.urdf_pos[18][1])*self.M*9.81
            mgc2 = (self.c_cm[1]-robot.urdf_pos[41][1])*self.M*9.81
        ...
        for t in np.linspace(self.t_set+self.t_impact,self.t_stop,int((self.t_stop-self.t_set-self.t_impact)/self.t_step2)):
           ...
           # p.setRealTimeSimulation(0)
            ...
            if t > self.t_set+self.t_rest and t < self.t_impact+self.t_set+self.t_rest:                             #This combination works the best comparison to v=[000000]
              ...
              # p.applyExternalForce(robot.robot_id,-1,[-0,-9.5,-0],[0,0,0],flags=p.LINK_FRAME) #keep this value
               #p.applyExternalForce(robot.robot_id,-1,[8,-8,-0],[0,0,0],flags=p.LINK_FRAME)
               #self.F_app = np.array([0,-21,0])
               self.F_app = np.array([0,-0,0]) #works for box mass 4
               p.applyExternalForce(robot.robot_id,-1,self.F_app,[0,0,0],flags=p.LINK_FRAME)
          ...
          #  p.changeDynamics(robot.robot_id ,  -1, mass=0.13)  #changing mass of base of biped
        ...
        #self.h = np.array([m_vcm - self.M*self.v_base]).T@np.linalg.pinv([dth]).T
           ...
           # p.setJointMotorControlArray(robot.robot_id,robot.rev,p.POSITION_CONTROL,targetPositions = self.thdes,
            ...
            if t<7:
                ...
                p.setJointMotorControl2(robot.robot_id, jointIndex = robot.rev[4],
                                        controlMode = p.TORQUE_CONTROL, 
                                        force =  0*self.jtorques[4] - ankle_torque_1+beta1*mgc1)

                p.setJointMotorControl2(robot.robot_id, jointIndex = robot.rev[10],
                                        controlMode = p.TORQUE_CONTROL, 
                                        force =  0*self.jtorques[10] - ankle_torque_2+beta2*mgc2) 
