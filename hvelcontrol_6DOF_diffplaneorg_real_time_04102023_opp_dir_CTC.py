...
import time
import numpy as np
from biped_6DOF_140923 import Biped
...
#       self.timearray = np.linspace(self.t_set,self.t_stop,(self.t_stop-self.t_set)*240)
    ...
    def getNormalForce_modi(self):
        ...
        f1a = p.getContactPoints(robot.robot_id,robot.planeID,4,-1)

        f2a = p.getContactPoints(robot.robot_id,self.plane_2,1,-1)

        f1b = p.getContactPoints(robot.robot_id,robot.planeID,5,-1)

        f2b = p.getContactPoints(robot.robot_id,self.plane_2,2,-1)
       ...
       # maxlenf1 = max(len(f1a),len(f1b))
        ...
        ankle_joint_pos = robot.urdf_pos[4]
        ...
        ankle_joint_pos = robot.urdf_pos[1]    
    ...
    def chooseeig(self,v,thd, t):
        ...
        xnorm=np.zeros((len(robot.rev)-3,1))
    ...
    def runSim(self):
        ...
        self.urdf_path1 = "/home/udayan/Desktop/Alinjar/Biped Pybullet/bipedal_03082023_12DOF_and_6_DOF/bipedal_12_01/box_small_13092023.urdf"

        self.plane_2  = p.loadURDF(self.urdf_path1,useFixedBase= False, basePosition=[-0.05+2,-0.05+2,0.025])
        ...
        #p.changeDynamics(robot.planeID ,  -1, lateralFriction=0.5)
        p.changeDynamics(robot.robot_id ,  -1, mass=0.2)  #changing mass of base of biped
        ...
        #vbase_r= robot.base_vel+self.skewM(robot.base_angvel)@(robot.urdf_pos[0]--robot.com_base_pos)
            vbase_l= self.v_base+self.skewM(robot.link_omega[0])@(robot.urdf_pos[0]-robot.com_base_pos)
          
            vbase_r= self.v_base+self.skewM(robot.link_omega[3])@(robot.urdf_pos[3]-robot.com_base_pos)
         #   x1=vbase_l+self.skewM(robot.link_omega[9])@(-robot.urdf_pos[9]+robot.com_pos[9])


            Mr = np.sum(robot.setLinkM()[0:3])
            Ml = np.sum(robot.setLinkM()[3:6])

            #M_base= robot.setLinkM()[16]+robot.setLinkM()[0]+robot.setLinkM()[8]

            M_base = robot.m_array[6]
       ...
       #m_vcm = np.sum(np.tile(robot.m_array,(3,1))*link_vell.T,axis=1)
        ...
        #self.h = np.array([m_vcm - self.M*self.v_base]).T@np.linalg.pinv([dth]).T
           ...
           # p.setJointMotorControlArray(robot.robot_id,robot.rev,p.POSITION_CONTROL,targetPositions = self.thdes,
            ...
            if t<self.t_set+ self.t_rest + self.t_impact+ 2:

               p.setJointMotorControl2(robot.robot_id, jointIndex = robot.rev[2] ,
                                        controlMode = p.TORQUE_CONTROL, 
                                        force =  self.jtorques[2] - ankle_torque_2)

               p.setJointMotorControl2(robot.robot_id, jointIndex = robot.rev[5] ,
                                        controlMode = p.TORQUE_CONTROL, 
                                        force =  self.jtorques[5] - ankle_torque_1) 
            ...
            ##ankle torque control
            mgc1 = (self.c_cm[1]-robot.urdf_pos[2][1])*self.M*10
            mgc2 = (self.c_cm[1]-robot.urdf_pos[5][1])*self.M*10
            ...
            if t<9:
                ...
                p.setJointMotorControl2(robot.robot_id, jointIndex = robot.rev[2],
                                        controlMode = p.TORQUE_CONTROL, 
                                        force =  0*self.jtorques[2] - ankle_torque_2+beta1*mgc1)

                p.setJointMotorControl2(robot.robot_id, jointIndex = robot.rev[5],
                                        controlMode = p.TORQUE_CONTROL, 
                                        force =  0*self.jtorques[5] - ankle_torque_1+beta2*mgc2) 
            ...
            #         p.setJointMotorControl2(robot.robot_id,robot.rev[3],p.TORQUE_CONTROL,force = hiptaustrat2)
