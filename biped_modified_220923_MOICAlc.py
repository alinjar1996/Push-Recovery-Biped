...
class Biped:
    ...
    def startSim(self,table=True):
        ...
        self.urdf_path = "Biped_12DOF_210923.urdf"
        self.robot_id = p.loadURDF(self.urdf_path,useFixedBase= False, basePosition=[1,1,0])
        self.mu =0.5 #0.2 for no sliding result
        ...
        p.resetDebugVisualizerCamera(cameraDistance = 0.7, cameraYaw= 35, cameraPitch = -10, cameraTargetPosition = [1,1,0.4])
        #print(p.getDebugVisualizerCamera()[8:11]) #yaw, pitch, distance
    ...
    def getJointState(self):
      ...
      #  print("jdetails:",self.jdetails)
        ...
        #print("jointt", jointt)
        # rev joints 4,7,12,16,17,24,27,30,35,39,40,47
        
        print("Lhipupperaxis",p.getJointInfo(self.robot_id,4)[13])
        print("Lhiploweraxis",p.getJointInfo(self.robot_id,7)[13])
        print("Lthighaxis",p.getJointInfo(self.robot_id,12)[13])
        print("Lshankaxis",p.getJointInfo(self.robot_id,16)[13])
        print("Lfootupperaxis",p.getJointInfo(self.robot_id,17)[13])
        print("Lfootloweraxis",p.getJointInfo(self.robot_id,24)[13])

        print("Rhipupperaxis",p.getJointInfo(self.robot_id,27)[13])
        print("Rhiploweraxis",p.getJointInfo(self.robot_id,30)[13])
        print("Rthighaxis",p.getJointInfo(self.robot_id,35)[13])
        print("Rshankaxis",p.getJointInfo(self.robot_id,39)[13])
        print("Rfootupperaxis",p.getJointInfo(self.robot_id,40)[13])
        print("Rfootloweraxis",p.getJointInfo(self.robot_id,47)[13])
    ...
    def getCOMPos(self):
        ...
        #self.urdf_transform = self.RR_modi(np.array([i[5] for i in link_data])) #in matrix
       ...
       # self.m_array = np.append(total_mass_array_12dof, base_mass)

      #  print("mass_array_base_at_end", self.m_array)
       ...
       # self.mass = np.array(i[0] for i in self.linkinfo)
        ...
        Lhip_upper_com = (total_mass_array[4]*self.com_pos[4]+total_mass_array[5]*self.com_pos[5]+total_mass_array[6]*self.com_pos[6])/(Lhip_mass_1)

        Lhip_lower_com = (total_mass_array[7]*self.com_pos[7]+total_mass_array[8]*self.com_pos[8]+total_mass_array[9]*self.com_pos[9]+
                          total_mass_array[10]*self.com_pos[10]+total_mass_array[11]*self.com_pos[11])/(Lhip_mass_2)
        ...
        Lfoot_upper_com = (total_mass_array[17]*self.com_pos[17]+total_mass_array[18]*self.com_pos[18]+total_mass_array[19]*self.com_pos[19]+
                      total_mass_array[20]*self.com_pos[20]+total_mass_array[21]*self.com_pos[21]+total_mass_array[22]*self.com_pos[22]+
                      total_mass_array[23]*self.com_pos[23])/(Lankle_mass_1)

        Lfoot_lower_com =  (total_mass_array[24]*self.com_pos[24]+total_mass_array[25]*self.com_pos[25])/(Lankle_mass_2) 
        ...
        """
        Rhip_upper_com = (total_mass_array[27]*self.com_pos[27]+total_mass_array[28]*self.com_pos[28]+total_mass_array[29]*self.com_pos[29])/(Rhip_mass_1)
        
        Rhip_lower_com = ( total_mass_array[30]*self.com_pos[30]+total_mass_array[31]*self.com_pos[31]+total_mass_array[32]*self.com_pos[32]+
                           total_mass_array[33]*self.com_pos[33]+total_mass_array[34]*self.com_pos[34])/(Rhip_mass_2)
        ...
        Rfoot_upper_com = (total_mass_array[40]*self.com_pos[40]+total_mass_array[41]*self.com_pos[41]+total_mass_array[42]*self.com_pos[42]+
                        total_mass_array[43]*self.com_pos[43]+total_mass_array[44]*self.com_pos[44]+total_mass_array[45]*self.com_pos[45]+
                        total_mass_array[46]*self.com_pos[46])/(Rankle_mass_1)

        Rfoot_lower_com =    (total_mass_array[47]*self.com_pos[47]+total_mass_array[48]*self.com_pos[48])/(Rankle_mass_2)            
...
#        print("Base inertia matrix ", base_inertia_matrix)
        ...
        #print("self.RR_modi(self.urdf_orien[1])", self.RR_modi(self.urdf_orien[1]))
        ...
        Q_base = self.RR_modi(self.base_ori)
        base_inertia_matrix = Q_base@base_inertia_matrix@Q_base.T

        MOI_Torso =  np.zeros((3,3))
    
        for i in range(0,27):
            if i in range(0,4) or i in range (26,27):
                Qi = self.RR_modi(self.urdf_orien[i])
                c_vec_Torso = base_com - self.com_pos[i]
                c_sqaure_torso= self.equicmatrix(c_vec_Torso)
                I_matrix_Torso = ((Qi@(self.inertia_all_matrix[i])@Qi.T +((total_mass_array[i])*(c_sqaure_torso))))
                MOI_Torso += I_matrix_Torso

        base_com_displaced = base_com - self.com_base_pos       
               
        MOI_Torso = base_inertia_matrix+ MOI_Torso + 0.021*self.equicmatrix(base_com_displaced)
        print("MOI of_Torso",I_matrix_Torso)
        

        # Inertia of Left hip 
        MOI_Lhip_upper =  np.zeros((3,3))
    
        for i in range(4,7):
            Qi = self.RR_modi(self.urdf_orien[i])
            c_vec_Lhip = Lhip_upper_com - self.com_pos[i]
            c_sqaure_Lhip= self.equicmatrix(c_vec_Lhip)
            I_matrix_Lhip_upper = ((Qi@(self.inertia_all_matrix[i])@Qi.T +((total_mass_array[i])*(c_sqaure_Lhip))))
            MOI_Lhip_upper += I_matrix_Lhip_upper

        print("MOI of left hip upper", MOI_Lhip_upper)

       # time.sleep(1)
        MOI_Lhip_lower =  np.zeros((3,3))
    
        for i in range(7,12):
            Qi = self.RR_modi(self.urdf_orien[i])
            c_vec_Lhip = Lhip_lower_com - self.com_pos[i]
            c_sqaure_Lhip= self.equicmatrix(c_vec_Lhip)
            I_matrix_Lhip_upper = ((Qi@(self.inertia_all_matrix[i])@Qi.T +((total_mass_array[i])*(c_sqaure_Lhip))))
            MOI_Lhip_lower += I_matrix_Lhip_upper

        print("MOI of left hip lower", MOI_Lhip_lower)
        ...
        for i in range(12,16):
            ...
            I_matrix_Lthigh = ((Qi@(self.inertia_all_matrix[i])@Qi.T +((total_mass_array[i])*(c_sqaure_Lthigh))))
       ...
       # time.sleep(1)
        ...
        for i in range(16,17):
            ...
            I_matrix_Lshank = ((Qi@(self.inertia_all_matrix[i])@Qi.T +((total_mass_array[i])*(c_sqaure_Lshank))))
       ...
       # time.sleep(1)
        ...
        MOI_Lfoot_upper =  np.zeros((3,3))

        for i in range(17,24):
            Qi = self.RR_modi(self.urdf_orien[i])
            c_vec_Lfoot = Lfoot_upper_com - self.com_pos[i]
            c_sqaure_Lfoot= self.equicmatrix(c_vec_Lfoot)
            I_matrix_Lfoot_upper = ((Qi@(self.inertia_all_matrix[i])@Qi.T +((total_mass_array[i])*(c_sqaure_Lfoot))))
            MOI_Lfoot_upper += I_matrix_Lfoot_upper

        print("MOI of left foot upper", MOI_Lfoot_upper)
      #  time.sleep(1)

        MOI_Lfoot_lower =  np.zeros((3,3))

        for i in range(24,26):
            Qi = self.RR_modi(self.urdf_orien[i])
            c_vec_Lfoot = Lfoot_lower_com - self.com_pos[i]
            c_sqaure_Lfoot= self.equicmatrix(c_vec_Lfoot)
            I_matrix_Lfoot_lower = ((Qi@(self.inertia_all_matrix[i])@Qi.T +((total_mass_array[i])*(c_sqaure_Lfoot))))
            MOI_Lfoot_lower += I_matrix_Lfoot_lower

        print("MOI of left foot lower", MOI_Lfoot_lower)

         # Inertia of Right hip
        MOI_Rhip_upper =  np.zeros((3,3))

        for i in range(27,30):
            Qi = self.RR_modi(self.urdf_orien[i])
            c_vec_Rhip = Rhip_upper_com - self.com_pos[i]
            c_sqaure_Rhip= self.equicmatrix(c_vec_Rhip)
            I_matrix_Rhip_upper = ((Qi@(self.inertia_all_matrix[i])@Qi.T +((total_mass_array[i])*(c_sqaure_Rhip))))
            MOI_Rhip_upper += I_matrix_Rhip_upper

        print("MOI of Right hip upper", MOI_Rhip_upper)
       # time.sleep(1)
        MOI_Rhip_lower =  np.zeros((3,3))

        for i in range(30,35):
            Qi = self.RR_modi(self.urdf_orien[i])
            c_vec_Rhip = Rhip_lower_com - self.com_pos[i]
            c_sqaure_Rhip= self.equicmatrix(c_vec_Rhip)
            I_matrix_Rhip_lower = ((Qi@(self.inertia_all_matrix[i])@Qi.T +((total_mass_array[i])*(c_sqaure_Rhip))))
            MOI_Rhip_lower += I_matrix_Rhip_lower

        print("MOI of Right hip lower", MOI_Rhip_lower)
        ...
        for i in range(35,38):
            ...
            I_matrix_Rthigh = ((Qi@(self.inertia_all_matrix[i])@Qi.T +((total_mass_array[i])*(c_sqaure_Rthigh))))
      ...
      #  time.sleep(1)
        ...
        for i in range(39,40):
            ...
            I_matrix_Rshank = ((Qi@(self.inertia_all_matrix[i])@Qi.T +((total_mass_array[i])*(c_sqaure_Rshank))))
     ...
     #   time.sleep(1)
        ...
        MOI_Rfoot_upper =  np.zeros((3,3))

        for i in range(40,47):
            Qi = self.RR_modi(self.urdf_orien[i])
            c_vec_Rfoot = Rfoot_upper_com - self.com_pos[i]
            c_sqaure_Rfoot= self.equicmatrix(c_vec_Rfoot)
            I_matrix_Rfoot_upper = ((Qi@(self.inertia_all_matrix[i])@Qi.T +((total_mass_array[i])*(c_sqaure_Rfoot))))
            MOI_Rfoot_upper += I_matrix_Rfoot_upper           

        print("MOI of Right foot upper", MOI_Rfoot_upper)

        MOI_Rfoot_lower =  np.zeros((3,3))

        for i in range(47,49):
            Qi = self.RR_modi(self.urdf_orien[i])
            c_vec_Rfoot = Rfoot_lower_com - self.com_pos[i]
            c_sqaure_Rfoot= self.equicmatrix(c_vec_Rfoot)
            I_matrix_Rfoot_lower = ((Qi@(self.inertia_all_matrix[i])@Qi.T +((total_mass_array[i])*(c_sqaure_Rfoot))))
            MOI_Rfoot_lower += I_matrix_Rfoot_lower           

        print("MOI of Right foot lower", MOI_Rfoot_lower)
     #   time.sleep(1)
    ...
    def drawPbText(self, text):
        if self.textptr != None:
                ...
                # pass
        self.textptr = p.addUserDebugText(text,textPosition = [1,1,0.5], textColorRGB = [1,0,0],textSize = 1)   
