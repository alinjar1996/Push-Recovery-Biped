...
class Biped:
    def __init__(self) -> None:
        ...
        self.startSim()
        self.setLinkM()
        self.allj= np.array([0,1,2,3,4,5])
        self.rev = np.array([0,1,2,3,4,5]) 
    ...
    def startSim(self,table=True):
       ...
       #/home/udayan/Desktop/Alinjar/Biped Pybullet/bipedal_20_07/bipedal_12_01/
        self.urdf_path = "BIPED_6DOF.urdf" #bipedal_2 has worked great so far
        ...
        alp = np.pi/2
        self.robot_id = p.loadURDF(self.urdf_path,useFixedBase= False, basePosition=[2,2,0.000], baseOrientation = [np.sin(alp/2)*np.cos(np.pi/2),np.sin(alp/2)*np.cos(np.pi/2),np.sin(alp/2)*np.cos(0),np.cos(alp/2)])
      #  self.robot_id_fixed = p.loadURDF(self.urdf_path,useFixedBase= True, basePosition=[0,0,0])
        ...
        p.resetDebugVisualizerCamera(cameraDistance = 0.6, cameraYaw= 35, cameraPitch = -10, cameraTargetPosition = [2,2,0.3])
    ...
    def getCOMPos(self):
        ...
        link_data  = p.getLinkStates(self.robot_id,[ i for i in range(6)],computeLinkVelocity=1)
    ...
    def getrobotCOMvelocity(self,modi_link_mass,modi_link_vel):
        ...
        modi_link_mass_right = modi_link_mass[0:3]

        modi_link_mass_left = modi_link_mass[3:6]

        modi_link_mass_base =  modi_link_mass[6]

        modi_link_vel_right = modi_link_vel[0:3]

        modi_link_vel_left = modi_link_vel[3:6]

        modi_link_vel_base = modi_link_vel[6]
    ...
    def drawPbText(self, text):
        if self.textptr != None:
                ...
                # pass
        self.textptr = p.addUserDebugText(text,textPosition = [2,2,0.5], textColorRGB = [1,0,0],textSize = 1)   
