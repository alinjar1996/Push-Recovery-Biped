...
class Biped:
    def __init__(self) -> None:
        ...
        self.startSim()
        self.setLinkM()
        self.allj= list(range(0,50))
        #self.rev = np.array([4,7,12,16,17,24,27,30,35,39,40,47]) #for normal biped
        self.rev = np.array([5,8,13,17,18,25,28,31,36,40,41,48])
    ...
    def startSim(self,table=True):
        ...
        self.urdf_path = "Biped_12DOF_torso_appended_210923.urdf"
        ...
        p.changeDynamics(self.planeID ,  -1, restitution=0.9)
        p.resetDebugVisualizerCamera(cameraDistance = 1, cameraYaw= 0, cameraPitch = 0, cameraTargetPosition = [1,1,0.4]) #frontal
        #p.resetDebugVisualizerCamera(cameraDistance = 0.9, cameraYaw= 0, cameraPitch = 90, cameraTargetPosition = [1,1,0.4]) #sagital
    ...
    def getrobotCOMvelocity(self,modi_link_mass,modi_link_vel):
        ...
        modi_link_mass_right = modi_link_mass[5:27]

        #modi_link_mass_left = modi_link_mass[27:49]
        modi_link_mass_left = modi_link_mass[28:50]
        ...
        modi_link_mass_base =  modi_link_mass[0]+ modi_link_mass[1]+ modi_link_mass[2]+ modi_link_mass[3]+modi_link_mass[4]\
        +modi_link_mass[27]+modi_link_mass[50]
        
        modi_link_vel_right = modi_link_vel[5:27]

        modi_link_vel_left = modi_link_vel[28:50]
        ...
        modi_link_vel_base =  (modi_link_mass[0]*modi_link_vel[0]+ modi_link_mass[1]*modi_link_vel[1]+ modi_link_mass[2]*modi_link_vel[2]+modi_link_mass[3]*modi_link_vel[3]+modi_link_mass[4]*modi_link_vel[4]+modi_link_mass[27]*modi_link_vel[27]+modi_link_mass[50]*modi_link_vel[50])/ modi_link_mass_base
    ...
    def getCOMPos(self):
        ...
        link_data  = p.getLinkStates(self.robot_id,[ i for i in range(50)],computeLinkVelocity=1)
       ...
       # print("self.linkinfo", self.linkinfo)
        ...
        Torso_mass = base_mass+ np.sum(total_mass_array[0:5])+total_mass_array[27]

        Lhip_mass_1 = np.sum(total_mass_array[5:8])
        Lhip_mass_2 = np.sum(total_mass_array[8:13])
        Lhip_mass = Lhip_mass_1 + Lhip_mass_2
        Lthigh_mass = np.sum(total_mass_array[13:17])
        Lshank_mass = np.sum(total_mass_array[17:18])
        Lankle_mass_1 = np.sum(total_mass_array[18:25])
        Lankle_mass_2 = np.sum(total_mass_array[25:27])
        ...
        Rhip_mass_1 = np.sum(total_mass_array[28:31])
        Rhip_mass_2 = np.sum(total_mass_array[31:36])
        Rhip_mass = Rhip_mass_1 + Rhip_mass_2
        Rthigh_mass = np.sum(total_mass_array[36:40])
        Rshank_mass = np.sum(total_mass_array[40:41])
        Rankle_mass_1 = np.sum(total_mass_array[41:48])
        Rankle_mass_2 = np.sum(total_mass_array[48:50])
        ...
        #print("total_mass_array_8dof", np.sum(total_mass_array_8dof))
       ...
       # self.mass = np.array(i[0] for i in self.linkinfo)
        ...
        base_com = (base_mass* self.com_base_pos + total_mass_array[0]*self.com_pos[0]+total_mass_array[1]*self.com_pos[1]+total_mass_array[2]*self.com_pos[2]+total_mass_array[3]*self.com_pos[3]+total_mass_array[4]*self.com_pos[4]+total_mass_array[27]*self.com_pos[27])/(Torso_mass)

        #print ("base_com", base_com)

        #beyond this line vaules have not been corrected after adding appended box at torso
    ...
    def drawPbline(self, point, ttz):
        if self.lineptr != None:
              p.removeUserDebugItem(self.lineptr)
                # pass
        self.lineptr = p.addUserDebugLine(point,[0,0,0],lineColorRGB=[0,1,0],lineWidth=5, lifeTime = ttz)    
